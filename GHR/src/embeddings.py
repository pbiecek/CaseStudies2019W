...
def load_word_vectors(embeddings_path):
    ...
    if os.path.isfile(embeddings_path + '.vec'):
        model = gensim.models.KeyedVectors.load_word2vec_format(embeddings_path + '.vec')
...
def load_embedding_model(data: str, emb_dir: str, emb_file: str, input_dim: int, vocab: Vocab, cuda: bool = False):

    new_emb_file = os.path.join(data, emb_dir.split("/")[-1]+"_"+emb_file + '_emb.pth')
    if os.path.isfile(new_emb_file) and torch.load(new_emb_file).size()[1] == input_dim:
        emb: Tensor = torch.load(new_emb_file)

    else:
        glove_vocab, glove_emb = load_word_vectors(os.path.join(emb_dir, emb_file))
